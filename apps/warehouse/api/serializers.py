...
class WarehouseSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Warehouse
        fields = ['id', 'user', 'user_info', 'name', 'address', 'capacity', 'metadata', 'status', 'created_at', 'updated_at']
    ...
    def _validate_field(self, value, min_length=2, max_length=100):
        if len(value) < min_length or len(value) > max_length:
            raise serializers.ValidationError("Field must be between 3 and 50 characters long.")
